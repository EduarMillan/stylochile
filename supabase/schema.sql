...
-- ============================================================================
-- Cursos, eventos y talleres del salón (entidad unificada con discriminador)
-- (También en supabase/migrations/0023_salon_events.sql)
-- ============================================================================
...
create policy "salon_events_owner_all" on public.salon_events for all
  using (exists (select 1 from public.salons s where s.id = salon_events.salon_id and s.owner_id = auth.uid()))
  with check (exists (select 1 from public.salons s where s.id = salon_events.salon_id and s.owner_id = auth.uid()));
